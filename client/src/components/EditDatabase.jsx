...
const EditDatabase = () => {
  ...
  useEffect(() => {
    ...
    const fetchDatabaseDetails = async () => {
      try {
        const response = await axios.get('https://utkarsh-fse-mha4s7stfa-uc.a.run.app/api/database/', {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      await axios.put('https://utkarsh-fse-mha4s7stfa-uc.a.run.app/api/database/', databaseDetails, {
        headers: { Authorization: `Token ${localStorage.getItem('token')}` },
      });
